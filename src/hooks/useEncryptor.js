const useEncryptor = (encryptedLength = 30) => {
  ...
  const encryptChar = (char) => {
    ...
    if (charIndex !== -1) {
      ...
      return characters[(charIndex + randOffset) % charCount] + characters[randOffset];
    }

    charIndex = extraChars.indexOf(char);
    if (charIndex !== -1) {
      return characters[charIndex % charCount] + characters[(charIndex / charCount) | 0];
    }
  ...
  const encryptId = (id) => {
    let lengthCode = lengthMap[id.length % charCount] + lengthMap[Math.floor(id.length / charCount) % charCount];
  ...
  const decryptId = (encryptedId) => {
    const originalLength = lengthMap.indexOf(encryptedId[0]) + lengthMap.indexOf(encryptedId[1]) * charCount;
    let decrypted = "";
    let index = 2;
    ...
    for (let i = 0; i < originalLength; i++) {
      ...
      if (charIndex === -1 || randOffset === -1) {
        ...
        let originalChar = characters[(charIndex - randOffset + charCount) % charCount];
        let extraCharIndex = characters.indexOf(originalChar) + randOffset * charCount;
